...
public class OrderRepositoryUnitTest extends BaseRepositoryTest {
    ...
    public void findByOrderId_test() throws Exception{
        ...
        Order returnOrder = orderRepository.findOrderInfoByOrderId(1L);
        assertEquals(2, returnOrder.getOrderItems().size());

    }

    @Test
    public void orderSearch_test() throws Exception {
        ...
        List<Order> orders = orderRepository.findAll(orderSearch.toSpecification());
        assertEquals(1, orders.size());


        // memberId null test
        orderSearch = OrderSearch.builder()
                                .orderState(OrderState.REQUEST)
                                .build();

        orders = orderRepository.findAll(orderSearch.toSpecification());
        assertEquals(1, orders.size());

        // OrderState null test
        orderSearch = OrderSearch.builder()
                                .memberId("JJM")
                                .build();
